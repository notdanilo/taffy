...
pub fn compute_grid_layout<Tree: LayoutGridContainer>(
    ...
    let final_content_size = Size {
        width: crate::util::sys::f32_max(item_content_size_contribution.width, columns.iter().map(|t| t.base_size).sum::<f32>() + padding_border_size.width),
        height: crate::util::sys::f32_max(item_content_size_contribution.height, rows.iter().map(|t| t.base_size).sum::<f32>() + padding_border_size.height),
    };
